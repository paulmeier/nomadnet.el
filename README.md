...
|---------------------------|-------------------------------------------------------|
| `M-x nomadnet`            | Dashboard with status and shortcuts to everything      |
| `M-x nomadnet-conversations` | List LXMF conversations; `RET` opens, `n` starts a new one |
| `M-x nomadnet-announces`  | Announce stream; `RET` browses a node or messages a peer |
| `M-x nomadnet-known-nodes`| Saved nodes; `RET` connects, `t` toggles trust        |
...
messages, `t` toggles trust of the peer. Composed messages are tagged as
